...
class QAgent():
    ...
    def test_model(self, model_path, num_episodes=10, render=True):
        ...
        self.speed_log = []  # Add this to store speeds
        ...
        for episode in range(num_episodes):
            ...
            episode_speeds = []  # Track speed for this episode
            ...
            while not done:
                ...
                episode_speeds.append(self.env.car.v)  # Save speed

                done = terminated or truncated
                state = torch.tensor(next_state).to(device)

            self.speed_log.append(episode_speeds)  # Store after episode ends
    ...
    def plot_speed_per_episode(self, filename="crossing_jaywalker_lex_speed.png"):
        import math
        plt.clf()

        episodes_to_plot = len(self.speed_log)
        cols = 2
        rows = math.ceil(episodes_to_plot / cols)

        fig, axs = plt.subplots(rows, cols, figsize=(10, 3 * rows))
        axs = axs.flatten()

        for i in range(episodes_to_plot):
            axs[i].plot(self.speed_log[i])
            axs[i].set_title(f"Episode {i + 1}")
            axs[i].set_xlabel("Time Step")
            axs[i].set_ylabel("Speed")
            axs[i].grid(True)

        for j in range(episodes_to_plot, len(axs)):
            axs[j].axis('off')

        fig.suptitle("Speed per Episode", fontsize=16)
