...
class Jaywalker:
    ...
    def __init__(self, jaywalker_speed = 0.0):
        ...
        # modifiche per aggiunta dinamica ostacoli
        self.n_lanes = 2
        ...
        self.counter_scenarios = 1 # modifica per scenari alternati
    ...
    def alternate_scenarios(self):
        ...
        if self.counter_scenarios > 4:
            self.counter_scenarios = 1
        # print(f"Scenario {self.counter_scenarios}")
    ...
    # --- Scenario 1: ostacolo distante (sorpasso possibile) ---
        if self.counter_scenarios == 1:
            ...
            speed = 0.1         # lento
            self.jaywalker_initial_pos = self.jaywalker_initial_pos_easy  # pedone in posizione facile

    # --- Scenario 2: ostacolo vicino (sorpasso critico) ---
        if self.counter_scenarios == 2:
            pos_x = self.jaywalker[0] + 5  # vicino al pedone
            speed = 0.5                    # veloce
            self.jaywalker_initial_pos = self.jaywalker_initial_pos_easy # pedone in posizione difficile

    # --- Scenario 3: ostacolo lontano + jaywalker hard (sorpasso facile) ---
        if self.counter_scenarios == 3:
            pos_x = self.dim_x  # molto lontano dal pedone
            speed = 0.1         # lento
            self.jaywalker_initial_pos = self.jaywalker_initial_pos_hard

    # --- Scenario 4: ostacolo vicino + jaywalker hard (sorpasso critico) ---
        if self.counter_scenarios == 4:
            pos_x = self.jaywalker[0] + 5  # vicino al pedone
            speed = 0.5                    # veloce
            self.jaywalker_initial_pos = self.jaywalker_initial_pos_hard
    ...
    def step(self, action):
        ...
        # modifiche per aggiunta dinamica macchina avversaria
